...
public class ECS {
     ...
     */
    protected EntitySystem[] systems;
     ...
     */
    public void process() {
        batchEnd = Math.min(entities.length, batchEnd);

        if (batchBegin >= entities.length) {
            batchBegin = 0;
            batchEnd = batchSize;
        }

        // Iterate through each registered entity system.
        for (int systemID = 0; systemID < this.systems.length; systemID++) {
            // Iterate through all entities in the ECS.
            for (int entityID = batchBegin; entityID < batchEnd; entityID++) {
                // Check if the entity exists and has the system flag set.
                if (this.entities[entityID] != null && systemFlags[systemID][entityID]) {
                    // Process the entity using the corresponding entity system.
                    this.systems[systemID].process(entityID);
                }
            }
        }
